package starling.utils
{
    ...
    public class AssetManager
    {
		...
		public var specialScaleFactors:Object = {};
                                      ...
                                      onProgress:Function, onComplete:Function):void
        {
            ...
            if (rawAsset is Class)
            {
                ...
                {
                    addTexture(name, Texture.fromBitmap(asset as Bitmap, mUseMipMaps, false, scaleFactorForName(name)));
                    onComplete();
                }
                else if (asset is ByteArray)
                {
                    ...
                    if (signature == "ATF")
                    {
                        addTexture(name, Texture.fromAtfData(asset as ByteArray, scaleFactorForName(name), 
            ...
            function onUrlLoaderComplete(event:Event):void
            {
                ...
                {
                    case "atf":
                        addTexture(name, Texture.fromAtfData(bytes, scaleFactorForName(name), mUseMipMaps, onComplete));
                        break;
                    case "fnt":
                    case "xml":
                        xmls.push(new XML(bytes));
            ...
            function onLoaderComplete(event:Event):void
            {
                ...
                if (content is Bitmap)
                    addTexture(name,
                        Texture.fromBitmap(content as Bitmap, mUseMipMaps, false, scaleFactorForName(name)));
                else
		...
		public function scaleFactorForName(name:String):Number
		{
			return specialScaleFactors[name] || mScaleFactor;
		}
    }
}